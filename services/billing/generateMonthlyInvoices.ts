    ...
    try {
      ...
      const invoice =
        await createInvoice({
          ...
          notes:
            `Monthly auto-generated invoice for ${billingPeriod}`,
      ...
      summary.rent_total +=
        Number(
          invoiceData.rent_total ?? 0
        );

      summary.utility_charges +=
        Number(
          invoiceData.utility_charges ?? 0
        );

      summary.previous_balances +=
        Number(
          invoiceData.previous_balances ?? 0
        );
